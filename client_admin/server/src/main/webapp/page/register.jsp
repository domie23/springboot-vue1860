                        ...
                        <div class="layui-input-block">
                            <select name="interest" id="user_group" lay-filter="user_group">
                                <option value=""></option>
                                                    ...
                                                    <div id="ordinary_manager" class="test1 diy_sf" style="display: none">
                                                                                                <div class="layui-form-item" id="ordinary_manager_job_no_box">
                                                <label class="layui-form-label">工号</label>
                                                <div class="layui-input-block">
                                                    <input type="text" name="title" lay-verify="title"
                                                           autocomplete="off" placeholder="请输入工号"
                                                           class="layui-input" id="ordinary_manager_job_no">
                                                </div>
                                            </div>
                                                                                                                    <div class="layui-form-item" id="ordinary_manager_full_name_box">
                                                <label class="layui-form-label">姓名</label>
                                                <div class="layui-input-block">
                                                    <input type="text" name="title" lay-verify="title"
                                                           autocomplete="off" placeholder="请输入姓名"
                                                           class="layui-input" id="ordinary_manager_full_name">
                                                </div>
                                            </div>
                                                                                                                    <div class="layui-form-item" id="ordinary_manager_gender_box">
                                                <label class="layui-form-label">性别</label>
                                                <div class="layui-input-block">
                                                    <select name="interest" id="ordinary_manager_gender" lay-filter="gender">
                                                        <option value=""></option>
                                                    </select>
                                                </div>
                                            </div>
...
<script>
    ...
    layui.use(['element', 'layer', 'util', 'jquery'], function () {
        ...
        // 数据
                                            ...
                                            var list_ordinary_manager_gender = ['男','女']

                            async function ordinary_manager_gender(list) {
                                var ordinary_manager_gender = document.querySelector("#ordinary_manager_gender")
                                var op1 = document.createElement("option");
                                op1.value = '0'
                                ordinary_manager_gender.appendChild(op1)
                                // 收集数据 长度
                                var count
                                // 收集数据 数组
                                var arr = []
                                ...
                                for (var i = 0; i < arr.length; i++) {
                                    ...
                                    op.value = arr[i]
                                    // 新增/Add节点
                                    ordinary_manager_gender.appendChild(op)
                                    layui.form.render("select");
                                }
                            }

                            ordinary_manager_gender(list_ordinary_manager_gender)
                            layui.form.on('select(ordinary_manager_gender)', function (data) {
            ...
            if (!username.value) {
                ...
                layer.msg('请选择用户组!');
                                        ...
                                        async function submit1() {
                                                                                        let ordinary_manager_job_no = document.querySelector('#ordinary_manager_job_no')
                                        from_data['job_no'] = ordinary_manager_job_no.value
                                                                                                            let ordinary_manager_full_name = document.querySelector('#ordinary_manager_full_name')
                                        from_data['full_name'] = ordinary_manager_full_name.value
                                                                                                            let ordinary_manager_gender = document.querySelector('#ordinary_manager_gender')
                                        from_data['gender'] = ordinary_manager_gender.value
                                                                        if (form1.user_group === "普通管理者") {
                                                                                        }
                        ...
                        if (from_data) {
                            ...
                            if (user_id) {
                                const {data: resss} = await axios.post(BaseUrl + `/api/ordinary_manager/add?`,
                                        from_data, {
                                            headers: {
                                                'x-auth-token': token,
                                                'Content-Type': 'application/json'
        ...
        function $check_list_data(){
                                            if (!$check_register_field('add','job_no','/ordinary_manager/view')){
                        document.querySelector('#ordinary_manager_job_no_box').style.display = "none"
                    }
                                        if (!$check_register_field('add','full_name','/ordinary_manager/view')){
                        document.querySelector('#ordinary_manager_full_name_box').style.display = "none"
                    }
                                        if (!$check_register_field('add','gender','/ordinary_manager/view')){
                        document.querySelector('#ordinary_manager_gender_box').style.display = "none"
                    }
                                            }
