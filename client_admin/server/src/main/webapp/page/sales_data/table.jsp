...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="sales_data" lay-filter="sales_data"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/sales_data/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/sales_data/table', 'del') || $check_action('/sales_data/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/sales_data/table', 'add') || $check_action('/sales_data/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/sales_data/view', 'get') || $check_action('/sales_data/view', 'set')) {
            detail_flag = true;
        }
 ...
 function open_tip(arr) {
    ...
    table.render({
        ...
        , cols: [[
                    ...
                    , {
                field: 'statistical_date', width: 180, title: '统计日期', sort: true
                                                    , templet: "<div>{{layui.util.toDateString(d.statistical_date, 'yyyy-MM-dd')}}</div>"
                                                                
            }
                                , {
                field: 'sales_quantity', width: 180, title: '售卖数量', sort: true
                                                                                
            }
                                , {
                field: 'profit_amount', width: 180, title: '利润金额', sort: true
    ...
    ,
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                                    ...
                                    if (user_group === '管理员' || $check_field('get', 'statistical_date', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='statistical_date']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'sales_quantity', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='sales_quantity']").css('display', 'none');
                    }
                                        if (user_group === '管理员' || $check_field('get', 'profit_amount', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='profit_amount']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/sales_data/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/sales_data/view', 'set') || $check_action('/sales_data/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(sales_data)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {

                            window.location.href = "./view_add.jsp?" + data.sales_data_id;
                    }
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('sales_data', BaseUrl + '/api/sales_data/del', 'sales_data_id', request, resetName, resetSelect        )
