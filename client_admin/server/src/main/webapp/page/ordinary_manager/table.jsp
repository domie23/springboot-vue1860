...
<div class="section1">
    ...
    <div class="table">
        <table class="layui-hide" id="ordinary_manager" lay-filter="ordinary_manager"></table>
...
<script src="../../js/axios.min.js"></script>
    ...
    layui.use(['element', 'layer', 'util'], function () {
        ...
        let url = BaseUrl + '/api/ordinary_manager/get_list?like=0'
        ...
        if (user_group == "管理员" || $check_action('/ordinary_manager/table', 'del') || $check_action('/ordinary_manager/view', 'del')) {
            deletes.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/ordinary_manager/table', 'add') || $check_action('/ordinary_manager/view', 'add')) {
            add.style.display = "block"
        }
        if (user_group == "管理员" || $check_action('/ordinary_manager/view', 'get') || $check_action('/ordinary_manager/view', 'set')) {
            detail_flag = true;
        }
        ...
        function getpath() {
            ...
            for (var i = 0; i < list_data.length; i++) {
                var o = list_data[i];
                if (o.path === "/ordinary_manager/table") {
                    console.log(o.path);
                    path1 = o.path
                        $get_power(o.path)
                }
 ...
 function open_tip(arr) {
        ...
        , cols: [[
            ...
            field: 'user_id', width: 200, title: '用户名', templet: function (d) {
                ...
                field: 'nickname', width: 150, title: '昵称', templet: function (d) {
                    ...
                    , {
                field: 'job_no', width: 180, title: '工号', sort: true
    ...
    ,
            ...
            if (res.count == null) { // 无数据时直接返回了
                return;
                                    ...
                                    if (user_group === '管理员' || $check_field('get', 'job_no', path1)) {
                        // console.log("显示")
                    } else {
                        $("[data-field='job_no']").css('display', 'none');
                    }
    ...
    ,
        ...
        open_tip(res.result.list);
            ...
            if (user_group == "管理员" || $check_pay('/ordinary_manager/table')) {
                for (var i = 0; i < res.result.list.length; i++) {
                    res.result.list[i].check_pay = true
                }
            }
            if (user_group == "管理员" || $check_action('/ordinary_manager/view', 'set') || $check_action('/ordinary_manager/view', 'get')) {
                for (var i = 0; i < res.result.list.length; i++) {
    ...
    ,
        ...
        table.on('tool(ordinary_manager)', function (obj) {
            var data = obj.data;
        if (obj.event === 'detail')
        {
        ...
        let resetSelect = []
        //下拉框重置/Reset参数
        fun('ordinary_manager', BaseUrl + '/api/ordinary_manager/del', 'ordinary_manager_id', request, resetName, resetSelect                        , undefined, undefined,true
        )
