        ...
        <div class="layui-container">
            <div class="layui-row">
                <form class="layui-form" action="">
                            ...
                            <button type="button" class="layui-btn" id="ordinary_manager">上传头像</button>
                            <div class="layui-upload-list">
                                <img class="layui-upload-img" id="ordinary_manager_img">
                                <p id="demoText"></p>
                            </div>
                            <div style="width: 95px;">
                                <div class="layui-progress layui-progress-big" lay-showpercent="yes"
                                     lay-filter="ordinary_manager">
                            ...
                            <div class="layui-input-block">
                                <select name="interest" lay-filter="state" id="state">
                                    ...
                                    <option value="4">已注销</option>
                                                                                        ...
                                                                                        <div class="layui-form-item" id="job_no_box">
                                            <label class="layui-form-label">工号</label>
                                            <div class="layui-input-block">
                                                <input type="text" name="title" lay-verify="title" autocomplete="off"
                                                       placeholder="请输入工号"
                                                       class="layui-input" id="job_no">
                                            </div>
                                        </div>
...
<script>
    var BaseUrl = baseUrl()
    let ordinary_manager_id = location.search.substring(1)
    layui.use(['upload', 'element', 'layer', 'laydate', 'layedit'], function () {
        ...
        // 提交按钮校验权限
        // if (   user_group == "管理员" ||$check_action('/ordinary_manager/view', 'add') || $check_action('/ordinary_manager/view', 'set')) {
        ...
        // style="display: none"
            ...
            var uploadInst = upload.render({
                elem: '#ordinary_manager'
                , url: BaseUrl + '/api/ordinary_manager/upload?' //此处用的是第三方的 http 请求演示，实际使用时改成您自己的上传接口即可。
                , headers: {
                    'x-auth-token': token
                }, before: function (obj) {
                    //预读本地文件示例，不支持ie8
                    obj.preview(function (index, file, result) {
                        $('#ordinary_manager_img').attr('src', fullUrl(BaseUrl,result)); //图片链接（base64）
                    });

                    element.progress('ordinary_manager', '0%'); //进度条复位
                ...
                , progress: function (n, elem, e) {
                    element.progress('ordinary_manager', n + '%'); //可配合 layui 进度条元素使用
                    if (n == 100) {
            ...
            layui.form.on('select(phone_state)', function (data) {
                ...
                console.log()
                                                                                    ...
                                                                                    let job_no = document.querySelector("#job_no")
                                                                                                                                                let full_name = document.querySelector("#full_name")
                                                                                                                                                            
            if (ordinary_manager_id !== '') {
                ...
                async function axios_get_4() {
                    ...
                    for (let key in form_data) {
                        if (key == 'avatar') {
                                ordinary_manager_img.src = BaseUrl + form_data.avatar
                        }
                    }
                    for (let key in form_data) {
                        if (key == 'email_state') {
                ...
                async function axios_get_5() {
                    const {data: rese} = await axios.get(BaseUrl + '/api/ordinary_manager/get_obj', {
                        params: {
                            user_id: ordinary_manager_id
                        }
                    })
            ...
            submit.onclick = async function () {
                ...
                if (!username.value) {
                    ...
                    layer.msg('密码不能为空');
                } else if (!ordinary_manager_id && (password.value.length > 16 || password.value.length < 5)) {
                    ...
                    if (ordinary_manager_id !== '') {
                        data_p['user_id'] = ordinary_manager_id
                    }
                    let data_p_param = {
                    ...
                    form_sub.user_id = idd
                            ...
                            form_sub.job_no = job_no.value
                            //文本
                                                                                    if (user_group === '管理员' || (form_sub['ordinary_manager_id'] && $check_field('set', 'job_no')) || (!form_sub['ordinary_manager_id'] && $check_field('add', 'job_no'))) {
                            }else {
                                $("#job_no").attr("disabled", true);
                                $("#job_no > input[name='file']").attr('disabled', true);
                            }
                                                                        //文本
                            form_sub.full_name = full_name.value
                            //文本
                                                                                    if (user_group === '管理员' || (form_sub['ordinary_manager_id'] && $check_field('set', 'full_name')) || (!form_sub['ordinary_manager_id'] && $check_field('add', 'full_name'))) {
                            }else {
                                $("#full_name").attr("disabled", true);
                                $("#full_name > input[name='file']").attr('disabled', true);
                            }
                                                                                                        if (user_group === '管理员' || (form_sub['ordinary_manager_id'] && $check_field('set', 'gender')) || (!form_sub['ordinary_manager_id'] && $check_field('add', 'gender'))) {
                            }else {
                                ...
                                $("#gender > input[name='file']").attr('disabled', true);
                                                ...
                                                if (ordinary_manager_id == '') {
                        console.log("新增/Add")
                        const {data: ress} = await axios.post(BaseUrl + '/api/ordinary_manager/add?', form_sub, {
                            headers: {
                                'x-auth-token': token,
                                'Content-Type': 'application/json'
                            }
                        ...
                        const {data: res_data2} = await axios.post(BaseUrl + '/api/ordinary_manager/set?ordinary_manager_id=' + form_sub.ordinary_manager_id, form_sub, {
                            headers: {
                                'x-auth-token': token,
                                'Content-Type': 'application/json'
                            }
